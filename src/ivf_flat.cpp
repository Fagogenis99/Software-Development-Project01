...
// Επιστρέφει τα indices των nprobe κοντινότερων centroids (σε αύξουσα απόσταση)
...
IVFIndexFlat build_ivf_flat(const Matrix& base, int kclusters, int seed, int train_subset) {
    ...
    // Χτίσε inverted lists από τις τελικές αναθέσεις (σε ΟΛΑ τα σημεία)
...
TopN ivf_flat_query_topN(const IVFIndexFlat& ivf,
    ...
    // 1) Διάλεξε τα nprobe κοντινότερα centroids
    std::vector<int> probes = top_nprobe_centroids(ivf.centroids, q, nprobe);

    // 2) Μάζεψε υποψηφίους από τις αντίστοιχες inverted lists και υπολόγισε αποστάσεις
    ...
    for (const auto& p : cand) {
        ...
        res.dists.push_back(std::sqrt(p.first)); // επιστρέφουμε ευκλείδεια (όχι τετράγωνο)
    }
    return res;
}
...
std::vector<int> ivf_flat_query_range(const IVFIndexFlat& ivf,
    ...
    // 2) Σάρωσε μόνο τις αντίστοιχες λίστες και κάνε threshold στο R
    for (int c : probes) {
        const auto& lst = ivf.lists[c];
        for (int id : lst) {
            float d2 = l2_sq(q, base.row(id), base.d);
