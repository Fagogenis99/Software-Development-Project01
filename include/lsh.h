...
namespace lsh {
    ...
    class HFunction {
        ...
        private:
            std::vector<double> v; //random projection vector N(0, 1)^d
            double t_off; //random offset in [0, w)
            double w_bucket; //bucket width
        };

    //class Gfunction represents a combination of k HFunctions to make g(p)
    class GFunction {
        public:
            GFunction(int dim, double w, int k, int tableSize); //constructor
            int hash(const std::vector<float>& p, unsigned int& id) const; 
        private:
    ...
    class LSH{
        public:
            LSH(int dim, int k, int L, double w, int tableSize = 10007, unsigned seed = 1); 
        ...
        private:
            ...
            int L_Tables; // L hash tables
            double w_size; 
            int table_size;
            unsigned seed_;

            std::vector<GFunction> gFunctions; //G Functions (one per table)
